...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse create(Integer shippingId, Integer userId) {
        ...
        Shipping shipping=shippingMapper.findShippingByShippingIdAndUsserId(shippingId,userId);
        if (shipping==null){
            return ServerResponse.createServerResponseByFail(12,"该地址不存在");
        }
        //2.根据userId查询购物车
        ...
        if (orderItemList==null||orderItemList.size()==0){
            return ServerResponse.createServerResponseByFail(4,"购物车为空");
        }
        ...
        orderItemMapper.insertBatch(orderItemList);//批量插入
        //6.扣库存
        reduceProducrStock(orderItemList);
        //7.清空购物车相应的数据
        cleanCart(cartList);
        //8.返回OrderVO
    ...
    public ServerResponse cancel(Long orderNO, Integer userId) {
        ...
        Order order=orderMapper.findOrderByUserIdAndOrderNo(orderNO,userId);
        if (order==null){
            return ServerResponse.createServerResponseByFail(1,"订单不存在");
        }
        //3.判断订单状态并取消
        if (order.getStatus()!=Const.OrderStatusEnum.ORDER_UNPAY.getCode()){
        ...
        if (count>0){
            return ServerResponse.createServerResponseBySucess("取消订单成功");
        }
        return ServerResponse.createServerResponseByFail(12,"取消订单失败");
    }
    ...
    public ServerResponse getOrderCartProduct(Integer userId) {
        ...
        //cartOrderItemVO.setImageHost();
        List<OrderItem>orderItemList= (List<OrderItem>) serverResponse.getData();
        List<OrderItemVO>orderItemVOList=Lists.newArrayList();
        if (orderItemList==null||orderItemList.size()==0){
            return ServerResponse.createServerResponseByFail(2,"购物车为空");
        }
        for (OrderItem orderItem:orderItemList){
    ...
    @Override
    public ServerResponse detail(Long orderNo, Integer userId) {
        if (orderNo==null){
            return ServerResponse.createServerResponseByFail(9,"参数不能为空");
        }
    ...
    private void reduceProducrStock(List<OrderItem> orderItemList){
        if (orderItemList!=null|orderItemList.size()>0){
            for (OrderItem orderItem:orderItemList){
    ...
    private Order createOrder(Integer userId, Integer shippingId, BigDecimal orderTotalPrice){
        ...
        order.setStatus(Const.OrderStatusEnum.ORDER_UNPAY.getCode());
    ...
    private ServerResponse getCartOrderItem(Integer userId,List<Cart> cartList){
        if (cartList==null||cartList.size()==0){
            return ServerResponse.createServerResponseByFail(5,"购物车为空");
        }
        ...
        for (Cart cart:cartList){
            ...
            if (product.getStock()<cart.getQuantity()){
                return ServerResponse.createServerResponseByFail(3,"库存不足");
            }
    ...
    public ServerResponse pay(Long orderNo, Integer userId) {
        if (orderNo==null){
            return ServerResponse.createServerResponseByFail(4,"订单号不能为空");
        }
        Order order=orderMapper.findOrderByOrderNo(orderNo);
        if (order==null){
            return ServerResponse.createServerResponseByFail(9,"订单号不存在");
        }
        return ServerResponse.createServerResponseBySucess(pay(order));
    ...
    public ServerResponse alipay_callback(Map<String, String> map) {
        ...
        Order order=orderMapper.findOrderByOrderNo(orderNo);
        if (order==null){
            return ServerResponse.createServerResponseByFail(15,"不是本订单的商品");
        }
        if (order.getStatus()>=Const.OrderStatusEnum.ORDER_PAYED.getCode()){
            return ServerResponse.createServerResponseByFail(14,"支付宝重复调用");
        }
        ...
        PayInfo payInfo1=payInfoMapper.selectByPrimaryKey(orderNo);
        if (payInfo1!=null){
            payInfo1.setPlatformStatus(trade_status);
            int count=payInfoMapper.updateByPrimaryKey(payInfo1);
            if (count>0){
                return ServerResponse.createServerResponseBySucess(true,null);
            }
        }else {
        ...
        return ServerResponse.createServerResponseByFail(14,"支付信息保存失败");
    }

    @Override
    public ServerResponse query_order_pay_status(Long orderNo, Integer userId) {
        if (orderNo==null){
            return ServerResponse.createServerResponseByFail(5,"参数不能为空");
        }
        Order order=orderMapper.findOrderByOrderNo(orderNo);
        if (order==null){
            return ServerResponse.createServerResponseByFail(9,"该订单不出在");
        }
        if (order.getStatus()==Const.OrderStatusEnum.ORDER_PAYED.getCode()){
            return ServerResponse.createServerResponseBySucess(true,null);
        }
        return ServerResponse.createServerResponseBySucess(false,null);
    }

    @Override
    public List<Order> closeOrder(String closeOrderDate) {
        List<Order>orderList=orderMapper.selectOrderByCreatTime(closeOrderDate);
        if (orderList==null||orderList.size()==0){
            return null;
        }
        for (Order order:orderList){
    ...
    public ServerResponse pay(Order order) {
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                .setNotifyUrl("http://5z5as4.natappfree.cc/order/alipay_callback.do")//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                String filePath = String.format("E:/bmpicture/qr-%s.png",
        ...
        return ServerResponse.createServerResponseByFail(8,"下单失败");
    }
